...
(function () {
  ...
  (function numericLimits() {
    const fields = [
      ...
      { id: 'id_weight_kg', min: 30,   max: 300,  label: 'Peso' },
      { id: 'id_height_cm', min: 100,  max: 250,  label: 'Altura' },
    ];
    fields.forEach(({ id, min, max, label }) => {
      ...
      el.addEventListener('blur', () => {
        ...
        if (isNaN(v) || v < min || v > max) {
          markError(el, `${label} debe estar entre ${min} y ${max}.`);
        } else {
          clearError(el);
        }
      });
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (res.ok && data.success) {
        /* Success — animate button to green then redirect */
        submitBtn.classList.remove('loading');
        submitBtn.style.background = 'linear-gradient(135deg,#1A5A3A,#3DBB77)';
        submitBtn.textContent = '✓ ¡Bienvenido guerrero!';
      ...
      submitBtn.disabled = false;
      console.error(err);
      /* Fallback — submit normal (no-AJAX) */
  ...
  function displayBackendErrors(errors) {
    Object.entries(errors).forEach(([field, msgs]) => {
      ...
      else console.warn('Backend error:', field, msg);
    });
    /* Focus first error */
